#ifndef REFRISYSTEM_RECOMMENDATION_IMPL_H
#define REFRISYSTEM_RECOMMENDATION_IMPL_H
...
void Recommendation<Item>::numericCheckerBasedSorter(vector<Item> &TargetList,
    ...
    sort(TargetList.begin(), TargetList.end(), [&](const Item &a, const Item &b) {
        ...
        else if (criteria == DESCENDING) return a_value > b_value;  // Sort in descending order based on the numeric value
        else throw runtime_error("numericCheckerBasedSorter criteria is not ASCENDING or DESCENDING");
    });
}

#endif //REFRISYSTEM_RECOMMENDATION_IMPL_H
